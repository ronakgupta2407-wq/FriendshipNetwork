...
function goTo(id) {
  ...
  if (id === 'dashPage') { setTimeout(syncComposerAv, 50); loadFeed(); loadSideStats(); }
}
    ...
    card.innerHTML = `
      ...
      <div class="person-info">
        ...
        <div style="display:flex;gap:6px;margin-top:2px;">
          ...
          <button class="btn-msg-friend" onclick="openSendMsg('${name.replace(/'/g,"\\'")}','${emoji}')" title="Send message">💬</button>
        </div>
      </div>`;
    grid.appendChild(card);
  });
...
function hideAllPanels() {
  ...
  document.getElementById('networkPanel').classList.remove('visible');
  document.getElementById('groupsPanel').classList.remove('visible');
  document.getElementById('activityPanel').classList.remove('visible');
}
...
document.getElementById('ql-network').addEventListener('click', () => { setActiveQL('ql-network'); showNetworkPanel(); });
document.getElementById('ql-groups').addEventListener('click', () => { setActiveQL('ql-groups'); showGroupsPanel(); });
document.getElementById('ql-activity').addEventListener('click', () => { setActiveQL('ql-activity'); showActivityPanel(); });
...
document.getElementById('networkPanelBack').addEventListener('click', showHomeView);
document.getElementById('groupsPanelBack').addEventListener('click', showHomeView);
document.getElementById('activityPanelBack').addEventListener('click', showHomeView);
  ...
  list.forEach(f => {
    ...
    card.innerHTML = `
      ...
      <div class="fp-actions">
        ...
        <button class="fp-remove-btn" onclick="removeFriend(${f.id},'${name.replace(/'/g,"\\'")}')" title="Remove friend">✕</button>
      </div>`;
    grid.appendChild(card);
  });
}
...
/* ════════════════════════════════════════════════════════════
   18. NETWORK MAP (Canvas Force-Directed Graph)
════════════════════════════════════════════════════════════ */
let networkNodes = [], networkEdges = [], netAnimId = null;

function showNetworkPanel() {
  hideAllPanels();
  document.getElementById('networkPanel').classList.add('visible');
  loadNetworkGraph();
}
...
async function loadNetworkGraph() {
  try {
    ...
    networkNodes = (data.nodes || []).map((n, i) => ({
      ...n, x: 200 + Math.cos(i * 0.8) * 150 + Math.random() * 80,
      y: 200 + Math.sin(i * 0.8) * 150 + Math.random() * 80,
      vx: 0, vy: 0, radius: 20 + Math.min(n.friends * 3, 15)
    }));
    networkEdges = data.edges || [];

    document.getElementById('netStatUsers').textContent = data.stats?.total_users || networkNodes.length;
    document.getElementById('netStatEdges').textContent = data.stats?.total_friendships || networkEdges.length;
    document.getElementById('netStatMyFriends').textContent = data.stats?.your_friends || 0;

    initNetworkCanvas();
  } catch (err) {
    document.querySelector('.network-canvas-wrap').innerHTML = `<p style="color:var(--error);text-align:center;padding:60px;">${err.message}</p>`;
  }
}
...
function initNetworkCanvas() {
  ...
  const colors = ['#f7376e','#ff8c42','#a78bfa','#4ade80','#38bdf8','#e91e8c','#1abc9c','#c0392b'];
  ...
  function getNodeById(id) { return networkNodes.find(n => n.id === id); }

  // Force simulation
  function simulate() {
    const k = 0.005, repulsion = 3000, damping = 0.85;
  ...
  function draw() {
    ...
    networkEdges.forEach(e => {
      const a = getNodeById(e.source), b = getNodeById(e.target);
      if (!a || !b) return;
      const isHovered = hoverNode && (hoverNode.id === a.id || hoverNode.id === b.id);
      ctx.beginPath();
      ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y);
      ctx.strokeStyle = isHovered ? 'rgba(167,139,250,0.6)' : 'rgba(255,255,255,0.08)';
    ...
    networkNodes.forEach((n, i) => {
      ...
      // Name label
      ctx.font = `bold ${isHover ? 12 : 10}px Nunito, sans-serif`;
      ctx.fillStyle = isHover ? '#fff' : 'rgba(237,233,252,0.7)';
      ...
      if (isHover) {
        ...
        ctx.fillText(`${n.friends} friends`, n.x, n.y + r + 26);
      }
    });
  ...
  canvas.addEventListener('mousemove', e => {
    ...
    for (const n of networkNodes) {
      ...
      if (Math.sqrt(dx*dx + dy*dy) < n.radius + 5) { hoverNode = n; break; }
    }
    canvas.style.cursor = hoverNode ? 'pointer' : (dragNode ? 'grabbing' : 'grab');
    if (dragNode) { dragNode.x = mx; dragNode.y = my; }
  });

  canvas.addEventListener('mousedown', e => {
    if (hoverNode) { dragNode = hoverNode; canvas.style.cursor = 'grabbing'; }
  });
...
/* ════════════════════════════════════════════════════════════
   19. EXPLORE GROUPS (DFS Connected Components)
════════════════════════════════════════════════════════════ */
function showGroupsPanel() {
  hideAllPanels();
  document.getElementById('groupsPanel').classList.add('visible');
  loadGroups();
}

async function loadGroups() {
  const grid = document.getElementById('groupsGrid');
  grid.innerHTML = '<p style="color:var(--muted);text-align:center;padding:40px;grid-column:1/-1;">Discovering groups via DFS…</p>';
  try {
    const data = await apiFetch('/api/groups');
    const groups = data.groups || [];

    document.getElementById('goTotalGroups').textContent = data.total_groups || groups.length;
    document.getElementById('goLargest').textContent = groups.length ? Math.max(...groups.map(g => g.size)) : 0;

    grid.innerHTML = '';
    if (!groups.length) {
      grid.innerHTML = '<p style="color:var(--muted);text-align:center;padding:50px;grid-column:1/-1;">No groups discovered. Add some friendships first!</p>';
      return;
    }

    const groupColors = [
      { bg: 'rgba(247,55,110,0.08)', border: 'rgba(247,55,110,0.2)', badge: '#f7376e' },
      { bg: 'rgba(167,139,250,0.08)', border: 'rgba(167,139,250,0.2)', badge: '#a78bfa' },
      { bg: 'rgba(74,222,128,0.08)', border: 'rgba(74,222,128,0.2)', badge: '#4ade80' },
      { bg: 'rgba(56,189,248,0.08)', border: 'rgba(56,189,248,0.2)', badge: '#38bdf8' },
      { bg: 'rgba(255,140,66,0.08)', border: 'rgba(255,140,66,0.2)', badge: '#ff8c42' },
    ];
    ...
    groups.forEach((g, i) => {
      ...
      const membersHTML = g.members.map(m =>
        `<span class="group-member-chip"><span class="group-member-emoji">${m.emoji}</span>${escapeHTML(m.name)}</span>`
      ).join('');

      card.innerHTML = `
        <div class="group-card-header">
          <div class="group-card-title">🏘️ Group ${g.group_id}</div>
          <span class="group-card-badge" style="background:${color.badge};color:#fff;">${g.size} members</span>
        </div>
        <div class="group-card-stats">
          <div class="group-stat"><span class="group-stat-num">${g.size}</span><span class="group-stat-lbl">Members</span></div>
          <div class="group-stat"><span class="group-stat-num">${g.internal_connections}</span><span class="group-stat-lbl">Connections</span></div>
          <div class="group-stat"><span class="group-stat-num">${(g.density * 100).toFixed(0)}%</span><span class="group-stat-lbl">Density</span></div>
        </div>
        <div class="group-members-list">${membersHTML}</div>`;
      grid.appendChild(card);
    });
  } catch (err) {
    grid.innerHTML = `<p style="color:var(--error);text-align:center;padding:40px;grid-column:1/-1;">${err.message}</p>`;
  }
}

/* ════════════════════════════════════════════════════════════
   20. ACTIVITY LOG (Stack — LIFO)
════════════════════════════════════════════════════════════ */
function showActivityPanel() {
  hideAllPanels();
  document.getElementById('activityPanel').classList.add('visible');
  loadActivityLog();
}

async function loadActivityLog() {
  const timeline = document.getElementById('activityTimeline');
  timeline.innerHTML = '<p style="color:var(--muted);text-align:center;padding:40px;">Loading activity log…</p>';
  try {
    const data = await apiFetch('/api/activity-log');
    const log = data.log || [];

    document.getElementById('aoTotal').textContent = data.total || log.length;

    timeline.innerHTML = '';
    if (!log.length) {
      timeline.innerHTML = '<p style="color:var(--muted);text-align:center;padding:50px;">No activity recorded yet.</p>';
      return;
    }

    const actionStyles = {
      'system':         { color: '#a78bfa', bg: 'rgba(167,139,250,0.12)', dot: '#a78bfa' },
      'user_added':     { color: '#4ade80', bg: 'rgba(74,222,128,0.12)', dot: '#4ade80' },
      'login':          { color: '#38bdf8', bg: 'rgba(56,189,248,0.12)', dot: '#38bdf8' },
      'friendship':     { color: '#f7376e', bg: 'rgba(247,55,110,0.12)', dot: '#f7376e' },
      'friend_request': { color: '#ff8c42', bg: 'rgba(255,140,66,0.12)', dot: '#ff8c42' },
      'unfriend':       { color: '#ef4444', bg: 'rgba(239,68,68,0.12)', dot: '#ef4444' },
      'post':           { color: '#a78bfa', bg: 'rgba(167,139,250,0.12)', dot: '#a78bfa' },
      'like':           { color: '#f7376e', bg: 'rgba(247,55,110,0.12)', dot: '#f7376e' },
      'comment':        { color: '#38bdf8', bg: 'rgba(56,189,248,0.12)', dot: '#38bdf8' },
      'event':          { color: '#4ade80', bg: 'rgba(74,222,128,0.12)', dot: '#4ade80' },
      'birthday':       { color: '#ff8c42', bg: 'rgba(255,140,66,0.12)', dot: '#ff8c42' },
    };

    log.forEach(entry => {
      const style = actionStyles[entry.action] || actionStyles['system'];
      const el = document.createElement('div');
      el.className = 'timeline-entry';
      el.innerHTML = `
        <div class="timeline-dot" style="background:${style.dot};"></div>
        <div class="timeline-content">
          <div class="timeline-message">${escapeHTML(entry.message)}</div>
          <div class="timeline-meta">
            <span class="timeline-action-badge" style="background:${style.bg};color:${style.color};">${entry.action}</span>
            <span>${formatPostTime(entry.timestamp)}</span>
          </div>
        </div>`;
      timeline.appendChild(el);
    });
  } catch (err) {
    timeline.innerHTML = `<p style="color:var(--error);text-align:center;padding:40px;">${err.message}</p>`;
  }
}

/* ════════════════════════════════════════════════════════════
   21. SIDEBAR STATS (live)
════════════════════════════════════════════════════════════ */
async function loadSideStats() {
  try {
    const data = await apiFetch('/api/stats');
    document.getElementById('sideStatMembers').textContent = data.total_users || '—';
    document.getElementById('sideStatConnections').textContent = data.total_friendships || '—';
    document.getElementById('postCountStat').textContent = data.total_posts || '0';
    document.getElementById('groupCountStat').textContent = data.total_groups || '0';
  } catch (err) { /* silent */ }
}
...
function loadDemoData() {
  ...
  [
    ...
    { id:Date.now()+3, author_name:'Ronak Gupta', text:'Working on our Friendship Network project using DSA concepts 🚀 AVL Trees, BFS, DFS — it\'s all coming together!', mood:'💪 Motivated', images:[], likes:8, liked_by_me:false, comments:3, created_at: new Date(Date.now()-7200000).toISOString(), saved_by_me:false },
